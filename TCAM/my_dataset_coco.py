...
class CocoDetection(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        """
        coco = self.coco
        img_id = self.ids[index]   #索引train的第index个
        ann_ids = coco.getAnnIds(imgIds=img_id)   #ann_ids 找出来这张图片的所有标注id号
        coco_target = coco.loadAnns(ann_ids)  #找出来这张图片的所有标注信息

        path = coco.loadImgs(img_id)[0]['file_name']  #找出来这个图片的真实名称
        img = Image.open(os.path.join(self.img_root, path)).convert('RGB')  #打开这张图片
...
if __name__ == '__main__':
    train = CocoDetection("/data/TCA Dataset", dataset="train")
    print(len(train))
    t = train[0]
