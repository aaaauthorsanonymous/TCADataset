...
def fastrcnn_loss(class_logits, box_regression, labels, regression_targets):
    ...
    # Calculate class loss information
    ...
    # Calculate the bounding box loss information
    box_loss = det_utils.smooth_l1_loss(
        # Gets the specified category box for the specified index proposal
...
def maskrcnn_loss(mask_logits, proposals, gt_masks, gt_labels, mask_matched_idxs):
    ...
    # Calculate the BCELoss between the predicted mask and the true gt_mask
...
class RoIHeads(torch.nn.Module):
    ...
    def add_gt_proposals(self, proposals, gt_boxes):
        ...
        """
        Splice the gt_boxes to the end of the proposal
        Args:
            proposals: boxes of rpn predictions for each image in a batch
            gt_boxes:  The true target bounding box for each image in a batch
    ...
    def select_training_samples(self,
        ...
        """
        Divide the positive and negative samples, and collect the label and bounding box regression information corresponding to gt
        The number of list elements is batch_size
        Args:
            proposals: rpn forecast boxes
            targets:
    ...
    def postprocess_detections(self,
        ...
        """
        Post-processing of the network's predictive data, including
            (1) Calculate the final bbox coordinates according to the proposal and predicted regression parameters
            (2) softmax was applied to the predicted category results
            (3) Crop the predicted boxes and adjust the out-of-bounds coordinates to the picture boundary
            (4) Remove all background information
            (5) Remove low probability targets
            (6) Remove small size objects
            (7) Perform nms processing and sort by scores
            (8) Return the top topk targets sorted according to scores
        Args:
            class_logits: Network prediction category probability information
            box_regression: boundary box regression parameter for network prediction
            proposals: Proposals for rpn output
            image_shapes: Width and height of each image before packaging into batch
